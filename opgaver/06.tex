...
For at gøre input-output konkret vises tre korte, illustrative eksempler, der følger principperne i tabel \ref{tab:esrs-e1-operationalisering}. Eksemplerne er ikke empiriske målinger, men konstruerede tal, der synliggør auditspor og sporbarhed.

\begin{mdframed}
\textbf{B2 varme (Scope 2)}\\
\textbf{Input:} Varmeforbrug 120.000 kWh; genindvundet varme 20.000 kWh; EF 0,20 kg CO$_2$e/kWh; vedvarende andel 10\%.\\
\textbf{Beregning:} Nettoforbrug = 100.000 kWh; brutto = 20.000 kg CO$_2$e; reduktion = 2.000 kg; netto = 18.000 kg.\\
\textbf{Output:} 18,0 t CO$_2$e registreres i beregningsspor.
\end{mdframed}

\begin{mdframed}
\textbf{C5 affald (Scope 3)}\\
\textbf{Input:} Affaldsmængde 500 kg; EF 1,2 kg CO$_2$e/kg.\\
\textbf{Beregning:} 500 $\times$ 1,2 = 600 kg CO$_2$e.\\
\textbf{Output:} 0,6 t CO$_2$e pr. fraktion.
\end{mdframed}

\begin{mdframed}
\textbf{S1 arbejdsmiljø (socialt)}\\
\textbf{Input:} 4 registrerede hændelser; 200.000 arbejdstimer i perioden.\\
\textbf{Beregning:} Frekvens = 4/200.000 = 2 hændelser pr. 100.000 arbejdstimer.\\
\textbf{Output:} Arbejdsmiljø-KPI for perioden med tilhørende auditspor.
\end{mdframed}
...
Dataindsamlingen er organiseret som et guidet modulforløb, hvor brugeren indtaster eller uploader data pr. modul. Input valideres gennem faste regler, og data gemmes med \textit{versionshistorik}, så ændringer kan spores over tid. Detaljer om datadefinitioner og kontrolpunkter fremgår af bilag \ref{app:teknisk-dokumentation}. Figur \ref{fig:b2-udfyldt} viser et udfyldt B2-modul, hvor enheder og perioder valideres i inputfasen.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{\detokenize{billeder af software/B2 – udfyldt formular.png}}
  \caption{Udfyldt B2-formular, der viser inputfelter og valideringskontekst.}
...
\begin{figure}[h]
  ...
  \begin{tabular}{c}
    \fbox{\parbox{0.78\textwidth}{\centering \textbf{Input}\\Datakilder fra drift, HR og økonomi}} \\
    $\downarrow$ \\
    \fbox{\parbox{0.78\textwidth}{\centering \textbf{Validering}\\Enheder, perioder og obligatoriske felter}} \\
    $\downarrow$ \\
    \fbox{\parbox{0.78\textwidth}{\centering \textbf{Beregning}\\Emissionsfaktorer og KPI-regler}} \\
    $\downarrow$ \\
    \fbox{\parbox{0.78\textwidth}{\centering \textbf{Auditspor}\\Kilde-ID, versioner og sign-off}} \\
    $\downarrow$ \\
    \fbox{\parbox{0.78\textwidth}{\centering \textbf{Output}\\PDF-preview og strukturerede data}} \\
  \end{tabular}
  \caption{Forenklet dataflow med lagdelt input, validering, beregning, auditspor og output.}
  \label{fig:dataflow-pipeline}
  \FigureSource{Egen fremstilling.}
\end{figure}

Automatisering sker gennem løbende gemmefunktion og beregning af indikatorer. Det reducerer risikoen for datatab og skaber et konsistent grundlag for rapportoutput. \textit{Auditsporet} registrerer ændringer og versioner og gør det muligt at efterprøve resultater og antagelser. Figur \ref{fig:b2-beregningstrace} viser et beregningstrace, hvor input, beregning og auditfelter hænger sammen.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{\detokenize{billeder af software/B2 – teknisk beregningstrace.png}}
  \caption{Beregningstrace for B2, der synliggør koblingen mellem input, beregning og auditspor.}
  \label{fig:b2-beregningstrace}
  \FigureSource{Egen fremstilling (skærmbillede fra MVP).}
\end{figure}
...
Dataindtastningen foregår modul for modul. Udvalgte eksempler er løftet ind i hovedteksten (figurer \ref{fig:b2-udfyldt}, \ref{fig:b2-beregningstrace} og \ref{fig:reportoutput-preview}), mens den fulde dokumentation af input- og review-flows er samlet i bilag \ref{app:supplerende-figurer-og-tabeller} (figurer \ref{fig:app-b2-tomt} til \ref{fig:app-review-topoverblik}).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{\detokenize{billeder af software/Review – PDF‑preview:sektion.png}}
  \caption{PDF-preview som rapportoutput, der viser hvordan resultater præsenteres for ledelse og interessenter.}
  \label{fig:reportoutput-preview}
  \FigureSource{Egen fremstilling (skærmbillede fra MVP).}
\end{figure}

I \textcitefull{EU2022CSRD} kræves, at rapportoutput genereres som PDF til ledelse og interessenter samt som strukturerede data til compliance i standardiserede formater (fx XBRL), hvilket gør output anvendeligt på tværs af beslutnings- og dokumentationssammenhænge. \textcite{Faccia2021XBRL} og \textcite{XBRLUS2022} fremhæver, at XBRL-tagging styrker maskinlæsbarhed og sammenlignelighed og er en forudsætning for automatiseret kontrol og audit. MVP'en genererer i denne version PDF-preview og et struktureret datasæt til intern brug, men iXBRL/XBRL-tagging blev ikke implementeret inden for tids- og scopesetup; det kræver taksonomi-mapping, namespace-håndtering og valideringsregler mod XBRL-taxonomien. Et eksempel på PDF-preview er gengivet i figur \ref{fig:reportoutput-preview}.
...
\end{equation*}
Tærsklen sættes til 90\% for at sikre næsten fuld dokumentation i minimumslisten; med fem auditfelter betyder det fuld udfyldelse i MVP-scenarierne, mens udvidede moduler kan tillade mindre mangler uden at miste auditabilitet.
