% IMPORTANT! In order for the document to compile, one needs to use XeLaTeX or LuaLaTeX as compiler.
% In Overleaf this can be done via Menu -> Settings -> Compiler -> Choose XeLaTeX/LuaLaTeX.
\documentclass[12pt]{article}

\input{preamble}

% Toggle for main-only PDF (compile full once to populate appendix labels).
\newif\ifmainonly
\mainonlyfalse
\ifmainonly
  \includeonly{opgaver/00,opgaver/01,opgaver/02,opgaver/03,opgaver/04,opgaver/05,opgaver/06,opgaver/07,opgaver/08,opgaver/09,opgaver/10}
\fi

% This change the content of the frontpage
\ptype{Selvtilrettelagt valgfag (15 ECTS)}
\author{Christian Kristensen}
\title{ESG-rapportering i sundhedssektoren}
\subtitle{Policy, organisering, økonomisk relevans og kommercialisering}
\date{Dato: 15. januar 2026}
\advisor{Vejleder: Karsten Vrangbæk}
\fpimage{picture.png} % Remove this command if no image is desired

\begin{document}

\maketitle

\onehalfspacing

\begin{table}[h]
\def\arraystretch{1.5}
\rowcolors{2}{black!5}{white}
\begin{tabularx}{\textwidth}{l X}
Institution: & Københavns Universitet, Fakultet for Sundheds- og Medicinvidenskab \\
Institut: & Institut for Folkesundhedsvidenskab \\
Uddannelse: & Bachelor i Sundhed og Informatik (5. semester) \\
Kursus: & SITA17001U Individuelt tilrettelagt studieenhed (15 ECTS) \\
Forfatter: & Christian Kristensen \\
KUID: & tdh522 \\
Titel og undertitel: & ESG-rapportering i sundhedssektoren: Policy, organisering, økonomisk relevans og kommercialisering \\
Vejleder: & Karsten Vrangbæk \\
Prøveform: & Skriftlig rapport \\
Bedømmelse: & Bestået / Ikke bestået \\
Maks. omfang: & 40 sider (Regnet til \RawCharCount\ tegn m. mellemrum; ca.\ \RawPageCount\ normalsider) \\
Bilag: & Indgår i rapporten og afleveres også som separat bilagsdokument (ikke medregnet i omfang) \\
Dato: & 15. januar 2026
\end{tabularx}
\end{table}

\section*{AI-deklaration}
Jeg erklærer, at AI kun er blevet brugt til grammatisk rettelse. Der er ikke anvendt AI til at generere indhold, argumentation eller struktur.
\newpage

\tableofcontents
\clearpage

\include{opgaver/00}
\include{opgaver/01}
\include{opgaver/02}
\include{opgaver/03}
\include{opgaver/04}
\include{opgaver/05}
\include{opgaver/06}
\include{opgaver/07}
\include{opgaver/08}
\include{opgaver/09}
\include{opgaver/10}

\clearpage
\printbibliography[heading=bibintoc,title={Referencer}]

\clearpage
\appendix
\include{opgaver/11}

\end{document}
